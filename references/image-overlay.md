...
The default Claude Warm Editorial answer is **no mask**. Real editorial covers (Kinfolk, Cereal, Apartamento, Monocle, Anthropic's own blog) almost never lay a uniform black gradient over a hero photo. They rely on **photo selection + composition** so text lands in a quiet zone naturally. If you cannot find such a photo, the answer is usually to swap the photo, not to caulk over a bad one with a 70%-black scrim.
...
If either test fails, **the photo is wrong for C01 image-led cover**. Fall back to C03 (split-layout cover with photo in a frame), or shoot/source again. Do not "fix it with a mask."
...
2. **Image-toned, not black.** Sample a dark tone already in the photo and use it as the tint color. Forest → deep moss `#1a2818`. Dusk → tea-rose dusk `#4a2638`. Snow → cool grey-blue `#2a3438`. Claude navy → `#181715`. Pure black `#000` reads as "annotation layer," not "atmospheric depth."
...
.hero-bleed::after {
  ...
  background: radial-gradient(
    ...
    rgba(24, 23, 21, 0.28) 0%,  /* Claude navy tone, soft peak */
    rgba(24, 23, 21, 0.0) 100%);
  pointer-events: none;
}
```
...
- **Uniform full-canvas vertical falloff** (the `rgba(0,0,0,.55) → .10 → .10 → .80` pattern). That is game-key-art treatment, not editorial.
- **Pure black mask color**. Always image-toned (forest, dusk, snow, Claude navy, sepia).
...
- **Asymmetric placement.** Titles offset to one side / one corner read more confident than dead-centered ones. The Kinfolk / Cereal / Anthropic-blog habit.
...
- **Move text off the image entirely** — title above, photo below.
