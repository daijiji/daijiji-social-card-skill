...
```
P1   C01 Image-Led Cover    ← single best photo, full bleed, restrained title
P2   C05 / C10 / C11        ← text-heavy: ledger / checklist / mini-data — eye rests
P3   C02 Field-Note Photo   ← second-best photo, in a frame, with caption
P4-N C02 / C11              ← more field notes or marginalia essays
P_last  C07 / C04           ← closing note or pull quote
```

Hard rule: **never two C01 image-led covers in a row**. After a full-bleed cover the eye needs to settle on text before another big image. Two image-led covers back-to-back read as "we couldn't decide which photo to use."

When this pattern does NOT apply:

- News / tutorial / data / review posts — the cover is a hook line, not a photo. Use C01 text-led variant or C03.
- Posts with one good photo only — use C01 for P1, then C02 once on P3-4 with that same photo or a different angle. Do not pad.

For the cover title placement and typography on a great photo, see `references/layout-recipes.md` C01 (4 modes: 顶压底沉 / 侧栏立柱 / 角落徽章 / 下沉条带) and `references/image-overlay.md` (subject map + localized tint fallback).
...
```text
Claude Warm Editorial conceptual product image, AI assistant interface on a warm cream desk, soft coral accent, deep navy product surface, no text, no logo, 3:4.
```
...
The Anthropic spike-mark is the brand's visual signature. Use it sparingly:

- Once on the cover, near the issue row or as a small mark above the title.
- Once on the closing page, near the sign-off.
- Never on every page — it loses meaning through repetition.

See `references/components.md` for the SVG markup.
