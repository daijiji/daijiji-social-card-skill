...
5. Convert the table to C08 Tall Ledger.
6. If there are only 2-3 points, use C04 Pull Quote or C09 Atmospheric Thesis instead of a table.

## Fixes For Sparse Text

If a page has only one core sentence:

- Use C09 Atmospheric Thesis with visible paper-wash + grain.
- Make the sentence the hero, not a small paragraph.
- Add one small source/context row.
- Use Dark Product or Midnight Claude theme if the package needs rhythm.
