...
**4. Drop English unless it carries the verb.** Mixed CJK + Latin at large size is a Claude Warm Editorial trick that takes a lot of practice. For 1:1 short titles, prefer pure Chinese. Exceptions: brand names (`AI`, `Mac`, `iOS`, `Claude`), domain terms with no Chinese equivalent (`PPT`, `MCP`).

**5. Add a small subtitle only if needed for disambiguation.** Most 1:1 covers don't need one. If you must add it, use `.kicker` or `.meta` size (18-21px mono), never a second `.h-xl`.
...
```html
<h1 class="h-display" style="font-size:520px; line-height:0.86;">家</h1>
<p class="meta">从北京搬到大理</p>
```
...
When the numerical fact is the news, lead with it. Use `.num-mega` (200px) over a 56px serif subtitle.
...
- **Three-line titles.** If your draft wraps to 3 lines on `.poster.square`, shorten the copy — never shrink `.h-display` below 108px. Three lines collapses into a paragraph.
...
Type scale defaults on square boards (from seed template):

| Class       | Default size | Use                                |
| ----------- | ------------ | ---------------------------------- |
| `.h-display`| 108px        | Pattern A or B, 1-2 lines          |
| `.h-xl`     |  78px        | Subtitle or secondary headline     |
| `.h-md`     |  48px        | Section title                      |
| `.num-mega` | 168px        | Pattern D leading number           |
| `.pullquote`|  56px        | Italic serif quote                 |

If a single-line short title is ≤ 4 Chinese chars on square, you can push `.h-display` to 160px without overflow.
