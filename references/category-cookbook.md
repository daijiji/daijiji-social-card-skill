...
- **Theme** — which of the 5 Claude Warm Editorial themes fits.
- **Recipes** — which layout IDs (C01-C14) are first reach.
...
- **Recipes**: C01 (text-led cover, no photo dominance) **or C01 image-led variant (when the user has 1+ great photos)**, C02 (field-note photo), C11 (marginalia essay), C07 (closing note), C05 (checklist for itinerary). For image-heavy submissions use the C01 → C05/C10 → C02 sequence in `references/content-planning.md`.
- **Theme**: Claude Canvas (default warm) for warm-tone destinations, Forest Warm for mountain/wilderness, Midnight Claude for night/cinematic travel.
...
**Strongest fit.** This is what Claude Canvas + Dark Product was made for.

- **Recipes**: C03 (cover), C10 (comparison), C05 (checklist), C06 (pipeline), C07 (takeaway ledger), C09 (KPI/atmospheric thesis), C11 (stacked ledger), C12 (matrix + hero stat).
- **Theme**: Claude Canvas for general workplace, Dark Product for code/technical content, Coral Callout for single-page emphasis (e.g. "one big claim" cover).
- **Text scheme**: Text-only or text-with-diagram. Almost never text-on-photo (workplace photos read as stock).
- **Image source**: Avoid stock business photos. If you need an image, prefer a diagram, a screenshot of a real artifact (Notion, Linear, Figma, Claude), or omit. Generated images rarely add value here.
...
- **Recipes**: C01 (cover with full-bleed art), C08 (boss tier ledger), C07 (takeaway ledger), C11 (chapter timeline), C13 (build before/after).
- **Theme**: Midnight Claude for atmospheric games (黑神话 / Elden Ring). Dark Product for esports / competitive data ("胜率 / KDA / 出装").
...
- **Recipes**: C04 (pull quote — for memorable lines), C10 (evidence feature — for scene analysis), C11 (marginalia essay), C10 (two signals — for comparison reviews), C12 (matrix — for film weeklies).
- **Theme**: Midnight Claude or Dark Product. Letterboxd visual vocabulary fits Claude Warm Editorial naturally — serif title, italic original-language subtitle, monospace metadata.
...
- **Recipes**: C01 (image-led cover when finished-dish photo is hero-worthy) → C11 (ingredient/price ledger) → C14 (cooking steps pipeline) → C02 (extra dish detail). Also C05 (checklist for shopping list).
- **Theme**: Claude Canvas feels like a cookbook. Coral Callout works for "cost-per-serving" data posts.
...
**Pitfalls**:
1. **Excited recipe voice** ("超绝!!!"). Claude Warm Editorial doesn't shout — let the dish do the talking.
...
- **Recipes**: C14 (tutorial step pipeline), C12 (product matrix), C11 (空瓶 / 复购 ledger), C10 (色号对比).
- **Theme**: Coral Callout for bright product reviews. Claude Canvas for editorial-feel beauty essays.
...
- Trending Y2K / 哥特 / dolly aesthetic decoration — outside the Claude Warm Editorial system.
...
- **Recipes**: C14 (workout pipeline), C11 (set/rep ledger), C09 (KPI tower for weekly volume), C13 (before/after — needs user photos).
- **Theme**: Coral Callout for energetic training posts. Forest Warm for "mindful movement / 山系跑步" essays.
...
- **Recipes**: C01 (image-led cover when user has a strong room photo) → C11 (item ledger with price) → C02 (detail crops). Also C13 (before/after — needs paired user photos), C11 (marginalia for floorplan annotation), C03 (essay-style decor manifesto).
- **Theme**: Claude Canvas for warm rustic homes. Dark Product for cool modern. Coral Callout for "rental hack data" posts.
...
- **Recipes**: C11 (marginalia essay — "this week I wore"), C12 (item matrix), C04 (style manifesto pull-quote), C11 (capsule wardrobe ledger).
- **Theme**: Midnight Claude or Claude Canvas for editorial-feel essays. Coral Callout for capsule-wardrobe data posts.
...
- **Recipes**: C04 (pull quote / thesis), C09 (atmospheric thesis), C11 (marginalia essay), C13 (hero question — for rhetorical-question covers), C07 (closing note).
- **Theme**: Midnight Claude or Claude Canvas. Magazine pace with restraint.
...
- **Content shape**: 3-5 pages. Cover question → essay split (C03) or atmospheric thesis → pull quote → closing note. Resist over-stretching to 9 pages.

### What does not work

- "梦核 / 氛围感" with soft fluorescent gradients, glowing ovals, hazy lights. Our anti-pattern list explicitly bans those decorations.
- Y2K / 早期网络回忆 / 千禧辣妹 aesthetic — outside Claude Warm Editorial.
- Heavy emoji / kaomoji captions. We use no emoji.

**Pitfalls**:
1. **Inspirational generic copy** ("愿你...愿我..."). Claude Warm Editorial demands specifics — name the day, the weather, the person.
2. **Pretty empty space** — page after page of one pullquote on cream. Risk: reads as filler. Mix with one C02 field photo or C11 marginalia to ground it.
...
- Claude Canvas for "I tested 5 products" → C12 matrix + C11 ledger.
- Claude Canvas for "this week's reading / watching" → C02 field notes.
