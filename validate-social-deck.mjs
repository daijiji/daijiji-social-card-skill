#!/usr/bin/env node
/*
 * validate-social-deck.mjs  —  Claude Warm Editorial edition
 *
 ...
 *   R2  footer collision      .issue-strip is position:absolute AND content above reaches into its band
 *   R3  bold display          .h-display / .h-xl / .h-md / .pullquote / .num-mega with computed weight >= 600
 *                              (Claude Warm Editorial "the larger, the lighter" — serif titles weight 400-500)
 ...
 *   R8  coral scarcity         more than one .card-coral OR more than one .btn-coral per poster
 *   R9  theme valid            <html data-theme> or <section data-theme> is one of the 5 allowed values
 */
...
if (!target) {
  console.error("usage: node validate-social-deck.mjs <task-dir|index.html>");
  process.exit(2);
}
...
const VALID_THEMES = ["claude-canvas", "coral-callout", "dark-product", "forest-warm", "midnight-claude"];

const sections = await page.$$("section.poster");
const report = [];

// "Obviously too small" floors — below these, text becomes unreadable at phone size.
// Seeds use 24px body, 28px lead — those are fine. Anything 4+px below is suspect.
const MIN_FONT = {
  body: 20,
  lead: 24,
  caption: 16,
...
const DISPLAY_CLASSES = ["h-display", "h-xl", "h-md", "pullquote", "num-mega", "num-xl"];
...
for (const s of sections) {
  ...
  const footIssue = await s.evaluate(el => {
    const foot = el.querySelector(".issue-strip, .foot");
    if (!foot) return null;
  ...
  if (footIssue) {
    fails.push({ rule: "R2", msg: `.issue-strip is position:absolute, ${footIssue.sel} extends ${footIssue.overlap}px past strip top`, fix: "switch .issue-strip to flex margin-top:auto (see style-system.md Anti-pattern C)" });
  }

  // R3 bold display — Claude Warm Editorial "the larger, the lighter"
  // Display titles should be weight 400-500. Weight 600+ at >=72px is a fail.
  ...
  for (const b of bolds) {
    fails.push({ rule: "R3", msg: `.${b.cls} "${b.text}" is ${b.size}px @ weight ${b.weight} — Claude Warm Editorial "larger = lighter" hard rule (max 500)`, fix: "remove inline font-weight; use the typed class default (400-500)" });
  }
  ...
  const textChecks = await s.evaluate((el, MIN) => {
    ...
    test(".kicker, .img-cap, .h-sub", "caption", MIN.caption);
    test(".meta, .label", "meta", MIN.meta);
    return out;
  }, MIN_FONT);
  ...
  if (cap) {
    const titles = await s.evaluate(el => {
      const out = [];
      for (const n of el.querySelectorAll(".h-display, .h-xl, .h-md")) {
        const cs = getComputedStyle(n);
        const size = parseFloat(cs.fontSize);
    ...
    for (const t of titles) {
      if (t.lines > cap.maxLines) {
        warns.push({ rule: "R6", msg: `.${t.cls} "${t.text}" renders ${t.lines} lines (cap ${cap.maxLines} on ${meta.board})`, fix: "switch to a cover recipe that allows taller titles, or trim copy" });
      }
    }
  }

  // R8 coral scarcity — at most one .card-coral and one .btn-coral per poster
  const coralCount = await s.evaluate(el => {
    const cards = el.querySelectorAll(".card-coral").length;
    const btns = el.querySelectorAll(".btn-coral").length;
    return { cards, btns };
  });
  if (coralCount.cards > 1) {
    fails.push({ rule: "R8", msg: `${coralCount.cards} .card-coral on one poster (max 1)`, fix: "coral is the accent, not the primary surface — switch extras to .card-cream or .card-navy" });
  }
  if (coralCount.btns > 1) {
    warns.push({ rule: "R8", msg: `${coralCount.btns} .btn-coral on one poster (max 1 recommended)`, fix: "use .btn-cream or .btn-navy for secondary actions" });
  }

  // R9 theme valid
  const themeOnPoster = meta.dataTheme;
  const themeOnHtml = await page.evaluate(() => document.documentElement.dataset.theme || "");
  const effectiveTheme = themeOnPoster || themeOnHtml;
  if (effectiveTheme && !VALID_THEMES.includes(effectiveTheme)) {
    fails.push({ rule: "R9", msg: `data-theme="${effectiveTheme}" is not one of ${VALID_THEMES.join(" / ")}`, fix: "use one of the 5 valid themes: claude-canvas, coral-callout, dark-product, forest-warm, midnight-claude" });
  }
...
const lines = [];
lines.push(`==== validate-social-deck (Claude Warm Editorial) ====`);
...
for (const { meta, fails, warns } of report) {
  if (fails.length === 0 && warns.length === 0) {
    lines.push(`[PASS]  ${meta.id} ${meta.dataId ? ` · ${meta.dataId}` : ""} · ${meta.board}${meta.dataTheme ? ` · ${meta.dataTheme}` : ""}`);
    continue;
  }
  const tag = fails.length ? "[FAIL]" : "[WARN]";
  lines.push(`${tag}  ${meta.id} ${meta.dataId ? ` · ${meta.dataId}` : ""} · ${meta.board}${meta.dataTheme ? ` · ${meta.dataTheme}` : ""}`);
...
lines.push("");
lines.push(`Legend: R1 overflow · R2 footer collision · R3 bold display (max 500) · R4 min font · R5 4-band density · R6 display cap · R7 figure margin · R8 coral scarcity · R9 theme valid`);
